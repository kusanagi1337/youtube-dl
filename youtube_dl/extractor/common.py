...
class InfoExtractor(object):
    ...
    Additionally, playlists should have "id", "title", and may also have any other relevant
    attributes with the same semantics as videos (see above), such as  "description", "uploader".
    ...
    Subclasses of InfoExtractor should re-define the _real_initialize()
    and _real_extract() methods and define a _VALID_URL regexp.
    Probably, they should also be added to the list of extractors.

    Subclasses may also override suitable() if necessary, but ensure the method
    signature is preserved and that this function imports everything it needs
    (except other extractors), so that lazy_extractors work correctly
    ...
    def extract(self, url):
        ...
        def get_temp_id():
            try:
                ...
                return hashlib.md5(encode_compat_str(url)).hexdigest()
        ...
        try:
            ...
            raise ExtractorError('A network error has occurred.', cause=e, expected=True, video_id=get_temp_id(url))
        except (KeyError, StopIteration) as e:
            raise ExtractorError('An extractor error has occurred.', cause=e, video_id=get_temp_id(url))
    ...
    def report_warning(self, msg, video_id=None, only_once=False):
        idstr = '' if video_id is None else '%s: ' % video_id
        msg = '[%s] %s%s' % (self.IE_NAME, idstr, msg)
        if only_once:
            sav_msg = 'WARNING: %s' % msg
            if sav_msg in self._printed_messages:
                return
        self._downloader.report_warning(msg)
        if only_once:
            self._printed_messages.add(sav_msg)
    ...
    def get_param(self, name, default=None):
        if self._downloader:
            return self._downloader.params.get(name, default)
        return default
    ...
    def _sort_formats(self, formats, field_preference=None):
        ...
        for f in formats:
            # from yt-dlp method IE._calculate_preference()
            ...
            if f.get('vcodec') == 'none':
                f['audio_ext'] = f['ext'] if f.get('acodec') != 'none' else 'none'
                f['video_ext'] = 'none'
            else:
                f['video_ext'] = f['ext']
                f['audio_ext'] = 'none'
            ...
            if f.get('tbr') is None:
                if f.get('vbr') is not None and f.get('abr') is not None:
                    f['tbr'] = f.get('vbr', 0) + f.get('abr', 0)
            else:
                if f.get('vcodec') != 'none' and f.get('vbr') is None:
                    f['vbr'] = f.get('tbr') - f.get('abr', 0)
                if f.get('acodec') != 'none' and f.get('abr') is None:
                    f['abr'] = f.get('tbr') - f.get('vbr', 0)

            # Set more filesize
            if f.get('filesize_approx') is None and f.get('filesize') is not None:
    ...
    def _parse_f4m_formats(self, manifest, manifest_url, video_id, preference=None, quality=None, f4m_id=None,
        ...
        for i, media_el in enumerate(media_nodes):
            ...
            format_id = '-'.join(map(compat_str, filter(None, (f4m_id, tbr or i))))
    ...
    def _m3u8_meta_format(self, m3u8_url, ext=None, preference=None, quality=None, m3u8_id=None):
        return {
            'format_id': ((compat_str(m3u8_id) + '-') if m3u8_id is not None else '') + 'meta',
            'url': m3u8_url,
            'ext': ext,
            'protocol': 'm3u8',
    ...
    def _parse_m3u8_formats(self, m3u8_doc, m3u8_url, ext=None,
        ...
        return self._parse_m3u8_formats_and_subtitles(
            m3u8_doc, m3u8_url, ext=ext, entry_protocol=entry_protocol,
            preference=preference, m3u8_id=m3u8_id, live=live)[0]
    ...
    def _parse_m3u8_formats_and_subtitles(
        ...
        if '#EXT-X-TARGETDURATION' in m3u8_doc:  # media playlist, return as is
            formats = [{
                'format_id': '-'.join(map(compat_str, filter(None, (m3u8_id, idx)))),
                'format_index': idx,
                'url': m3u8_url,
                'ext': ext,
        ...
        def extract_media(x_media_line):
            ...
            if media_url:
                ...
                formats.extend({
                    'format_id': '-'.join(map(compat_str, filter(None, (m3u8_id, group_id, name, idx)))),
                    'format_note': name,
                    'format_index': idx,
                    'url': manifest_url,
        ...
        for line in m3u8_doc.splitlines():
            if line.startswith('#EXT-X-STREAM-INF:'):
                ...
                for idx in _extract_m3u8_playlist_indices(manifest_url):
                    ...
                    f = {
                        'format_id': '-'.join(map(compat_str, filter(None, format_id))),
                        'format_index': idx,
                        'url': manifest_url,
                        'manifest_url': m3u8_url,
                        'tbr': tbr,
        ...
        for stream in ism_doc.findall('StreamIndex'):
            ...
            for track in stream.findall('QualityLevel'):
                ...
                if stream_type == 'text':
                    ...
                    formats.append({
                        'format_id': '-'.join(map(compat_str, filter(None, (ism_id, stream_name, tbr)))),
                        'url': ism_url,
                        'manifest_url': ism_url,
    ...
    def extract_comments(self, *args, **kwargs):
        ...
        def extractor():
            ...
            self.to_screen('Extracted {0} comments'.format(comment_count))
            return {
                'comments': comments,
    ...
    def _merge_subtitles(cls, *dicts, **kw):
        """ Merge subtitle dictionaries, language by language. """
        target = kw.pop('target', None) if kw else None
        assert not kw
        if target is None:
            target = {}
        for d in dicts:
